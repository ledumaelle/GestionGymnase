...
public class GestionReservationBdD
{
        ...
        try
	{
            ...
            if (jeuEnr.next())
            {
                ...
                UneReservation= new Reservation(UneSalle, jeuEnr.getDate("date"),jeuEnr.getInt("heure"),UneAssociation);
    ...
    public static ObservableList<Reservation> getReservationParAssociationEtSalle(Association pUneAssociation,Salle pUneSalle,Date pUneDate)
    {
        Connection conn; //connexion
	Statement stmt;
	ResultSet jeuEnr;
	String pilote = "org.gjt.mm.mysql.Driver";
	String url = new String("jdbc:mysql://localhost/gymnase");
        ObservableList<Reservation> lesReservationsParAssociationEtSalle = FXCollections.observableArrayList();
        try
	{
            Class.forName(pilote);
            conn = DriverManager.getConnection(url,"root","");
            stmt = conn.createStatement();			            
            jeuEnr = stmt.executeQuery("SELECT * FROM RESERVATION,SALLE,ASSOCIATION WHERE RESERVATION.NumSalle=SALLE.NumSalle AND ASSOCIATION.NumAssociation=RESERVATION.NumAssociation AND ASSOCIATION.NumAssociation = " +pUneAssociation.getNumAssociation()+" AND SALLE.NumSalle="+pUneSalle.getNumSalle()+" AND Date='"+pUneDate+"' ORDER BY Heure");
            Salle UneSalle;
            Association UneAssociation;
            Reservation UneReservation;
            while (jeuEnr.next())
            {
                ...
                UneAssociation= new Association(jeuEnr.getInt("NumAssociation"),jeuEnr.getString("NomAssociation"), jeuEnr.getString("AdresseAssociation"),jeuEnr.getString("CPAssociation"),jeuEnr.getString("VilleAssociation"),jeuEnr.getString("NomResponsable"));
                UneReservation= new Reservation(UneSalle, jeuEnr.getDate("date"),jeuEnr.getInt("heure"),UneAssociation);
                lesReservationsParAssociationEtSalle.add(UneReservation);
            } 
	...
	catch (ClassNotFoundException cnfe)
	{
        ...
        return lesReservationsParAssociationEtSalle;
    }
